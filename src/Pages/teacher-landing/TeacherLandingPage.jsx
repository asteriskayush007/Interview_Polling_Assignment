...
const TeacherLandingPage = () => {
  ...
  const askQuestion = () => {
    if (validateForm()) {
      ...
      // History me add
      let history = JSON.parse(localStorage.getItem("pollHistory") || "[]");
      history.push(pollData);
      localStorage.setItem("pollHistory", JSON.stringify(history));
  ...
  return (
    <>
      <button className="btn rounded-pill ask-question px-4 m-2" onClick={handleViewPollHistory}>
        <img src={eyeIcon} alt="" />
        View Poll history
      </button>
      ...
      <div className="container my-4 w-75 ms-5">
        ...
        <h2 className="fw-bold">Let's <strong>Get Started</strong></h2>
        ...
        <div className="mb-4">
          <div className="d-flex justify-content-between pb-3">
            ...
            <select className="form-select w-auto ms-3" value={timer} onChange={handleTimerChange}>
        ...
        <button className="btn add-options" onClick={addOption}>+ Add More option</button>
      </div>
      <hr />
      <button className="btn rounded-pill ask-question px-4 m-2" onClick={askQuestion}>
        Ask Question
      </button>
    </>
  );
